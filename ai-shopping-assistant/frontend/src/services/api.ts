...
export const ApiService = {
  ...
  async sendQuery(
    ...
    try {
      ...
      if (!AuthService.isAuthenticated() && !ActionTrackingService.isActionAllowed(ActionType.CHAT)) {
        throw new ApiError(
          'Guest action limit reached. Please log in to continue.',
          ApiErrorType.SERVER,
          403
        );
      }
      
      // Track this action only for guest users
      if (!AuthService.isAuthenticated()) {
        ActionTrackingService.trackApiAction(ActionType.CHAT);
      }
      
      // Handle authentication before making the request
      ...
      if (!response.ok) {
        ...
        if (response.status === 401 && AuthService.isAuthenticated()) {
          ...
          if (refreshed) {
            ...
            return await this.sendQuery(query, conversationHistory, timeout);
          }
        }
    ...
    if (error instanceof ApiError) {
      switch (error.type) {
        ...
        case ApiErrorType.SERVER:
          ...
          if (error.statusCode === 403 && error.message.includes('Guest action limit')) {
            return 'You have reached the guest usage limit. Please log in to continue using the assistant.';
          }
